...
def predict(args, input_size, model_name ='/home/kudzia/SHResNet/models/SHResNet_cross_1_2_100.pt', normalized=False):
    ...
    with torch.no_grad():
      data = torch.from_numpy(data_x.reshape(1, data_x.shape[0], data_x.shape[1], data_x.shape[2], -1))
      harm_SHCoeff = model(data)

    harm_SHCoeff = harm_SHCoeff.reshape(data.shape[1], data.shape[2], data.shape[3], -1)
...
def get_figure(args, harm_x):
    ...
    ax1.axis("off")
    ax1.title.set_text('Before harmonization')
    
    ax2 = fig.add_subplot(4, 3, 2)
    ax2.imshow(np.rot90(harm_x[:, :, 60, 10], 1), cmap='gray')
    ax2.axis("off")
    ax2.title.set_text('After harmonization')
...
def visual_measures(args, prediction, figure=True):
    ...
    if figure:
        ...
        fig3.savefig("/home/kudzia/results/SHResNet/result_RTOP_cross_" + args.net + "_" + str(args.b_x) + "_" + str(args.b_y) + ".svg")
        ...
        fig4.savefig("/home/kudzia/results/SHResNet/result_ASD_cross_" + args.net + "_" + str(args.b_x) + "_" + str(args.b_y) + ".svg")
        ...
        fig5.savefig("/home/kudzia/results/SHResNet/result_CVD_cross_" + args.net + "_" + str(args.b_x) + "_" + str(args.b_y) + ".svg")
        ...
        fig6.savefig("/home/kudzia/results/SHResNet/result_GFA_cross_" + args.net + "_" + str(args.b_x) + "_" + str(args.b_y) + ".svg")
...
def epoch_measures(args, input_size):
    ...
    if args.b_y == 1000:
        ...
        GFA_a = []
        for i in range(args.num_epochs):
            model = model_dir + str(args.net) + '_cross_' + str(int(args.b_x/1000)) + '_' + str(int(args.b_y/1000)) + '_' + str(
                (i + 1) * 10) + '.pt'
    ...
    plt.savefig("/home/kudzia/results/SHResNet/barplot_per_epoch_cross_"+str(args.net)+'_'+str(args.b_x)+'_'+str(args.b_y)+".svg")


def main():
    # Experiment settings
    parser = argparse.ArgumentParser(description='Harmoniation experiments')
    parser.add_argument('-n', '--net', default="SHResNet", help='net architecture')
