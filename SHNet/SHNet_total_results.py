...
def predict(args, input_size, model_name ='/home/kudzia/SHNet/models/SHNet_hansen_1_2_50.pt', normalized=False):
    ...
    true_x = SHBasis(filename_data, filename_bvals, filename_bvecs, args.order, args.b_x, normalized)
    ...
    model.eval()
    data = torch.from_numpy(np.transpose(data_x.reshape(data_x.shape[0],-1)))
    with torch.no_grad():
      harm_SHCoeff = model(data)

    harm_SHCoeff = np.transpose(harm_SHCoeff).reshape(data_x.shape[0], data_x.shape[1], data_x.shape[2], data_x.shape[3])
...
def get_figure(args, harm_x):
    ...
    im1 = axs[0,0].imshow(np.rot90(data[:, :, 10, 10], 1), cmap='gray')
    axs[0,0].axis("off")
    axs[0,0].title.set_text('Before harmonization')
    fig.colorbar(im1, ax=axs[0,0])
    
    im2 = axs[0,1].imshow(np.rot90(harm_x[:, :, 10, 10], 1), cmap='gray')
    axs[0,1].axis("off")
    axs[0,1].title.set_text('After harmonization')
    fig.colorbar(im2, ax=axs[0,1])
    
    im3 = axs[0,2].imshow(np.rot90(np.abs(harm_x[:, :, 10, 10] - data[:, :, 10, 10]), 1), cmap='gray')
    axs[0,2].axis("off")
    axs[0,2].title.set_text('Absolute Error')
    fig.colorbar(im3, ax=axs[0,2])
    '''
    ax4 = fig.add_subplot(4, 3, 4)
    ax4.imshow(np.rot90(data[:, 40, :, 10], 1), cmap='gray')
    ax4.axis("off")
    
    ax5 = fig.add_subplot(4, 3, 5)
    ax5.imshow(np.rot90(harm_x[:, 40, :, 10], 1), cmap='gray')
    ax5.axis("off")
    
    ax6 = fig.add_subplot(4, 3, 6)
    ax6.imshow(np.rot90(np.abs(harm_x[:, 40, :, 10] - data[:, 40, :, 10]), 1), cmap='gray')
    ax6.axis("off")
    
    ax7 = fig.add_subplot(4, 3, 7)
    ax7.imshow(np.rot90(data[40, :, :, 10], 1), cmap='gray')
    ax7.axis("off")

    ax8 = fig.add_subplot(4, 3, 8)
    ax8.imshow(np.rot90(harm_x[40, :, :, 10], 1), cmap='gray')
    ax8.axis("off")
    
    ax9 = fig.add_subplot(4, 3, 9)
    ax9.imshow(np.rot90(np.abs(harm_x[40, :, :, 10] - data[40, :, :, 10]), 1), cmap='gray')
    ax9.axis("off")
    '''
    im4 = axs[1,0].imshow(np.rot90(data[:, :, 15, 10], 1), cmap='gray')
    axs[1,0].axis("off")
    fig.colorbar(im4, ax=axs[1,0])
    
    im5 = axs[1,1].imshow(np.rot90(harm_x[:, :, 15, 10], 1), cmap='gray')
    axs[1,1].axis("off")
    fig.colorbar(im5, ax=axs[1,1])
    
    im6 = axs[1,2].imshow(np.rot90(np.abs(harm_x[:, :, 15, 10] - data[:, :, 15, 10]), 1), cmap='gray')
    axs[1,2].axis("off")
    fig.colorbar(im6, ax=axs[1,2])
    
    fig.savefig("/home/kudzia/results/SHResNet/data_hansen_" + args.net + "_" + ".svg")
...
def get_norm_figure(args, harm_x):
    ...
    true_x = SHBasis(filename_data, filename_bvals, filename_bvecs, args.order, args.b_x, normalized=True)
    data = true_x.data
    ...
    ax1.imshow(np.rot90(data[:, :, 10, 10], 1), cmap='gray')
    ax1.axis("off")
    ax1.title.set_text('Normalized before harm')
    
    ax2 = fig.add_subplot(4, 3, 2)
    ax2.imshow(np.rot90(harm_x[:, :, 10, 10], 1), cmap='gray')
    
    ax2.axis("off")
    ax2.title.set_text('Normalized after harm')
    
    ax3 = fig.add_subplot(4, 3, 3)
    ax3.imshow(np.rot90(np.abs(harm_x[:, :, 10, 10] - data[:, :, 10, 10]), 1), cmap='gray')
    ax3.axis("off")
    ax3.title.set_text('Absolute Error')
    
    ax4 = fig.add_subplot(4, 3, 4)
    ax4.imshow(np.rot90(data[:, 10, :, 10], 1), cmap='gray')
    ax4.axis("off")
    
    ax5 = fig.add_subplot(4, 3, 5)
    ax5.imshow(np.rot90(harm_x[:, 10, :, 10], 1), cmap='gray')
    ax5.axis("off")
    
    ax6 = fig.add_subplot(4, 3, 6)
    ax6.imshow(np.rot90(np.abs(harm_x[:, 10, :, 10] - data[:, 10, :, 10]), 1), cmap='gray')
    ax6.axis("off")
    
    ax7 = fig.add_subplot(4, 3, 7)
    ax7.imshow(np.rot90(data[10, :, :, 10], 1), cmap='gray')
    ax7.axis("off")

    ax8 = fig.add_subplot(4, 3, 8)
    ax8.imshow(np.rot90(harm_x[10, :, :, 10], 1), cmap='gray')
    ax8.axis("off")
    
    ax9 = fig.add_subplot(4, 3, 9)
    ax9.imshow(np.rot90(np.abs(harm_x[10, :, :, 10] - data[60, :, :, 10]), 1), cmap='gray')
    ax9.axis("off")
    
    ax10 = fig.add_subplot(4, 3, 10)
    ax10.imshow(np.rot90(data[:, :, 15, 10], 1), cmap='gray')
    ax10.axis("off")
    
    ax11 = fig.add_subplot(4, 3, 11)
    ax11.imshow(np.rot90(harm_x[:, :, 15, 10], 1), cmap='gray')
    ax11.axis("off")
    
    ax12 = fig.add_subplot(4, 3, 12)
    ax12.imshow(np.rot90(np.abs(harm_x[:, :, 15, 10] - data[:, :, 15, 10]), 1), cmap='gray')
    ax12.axis("off")
    
    fig.savefig("/home/kudzia/results/SHResNet/norm_hansen_cros_" + args.net + "_" + str(args.b_x) + "_" + str(args.b_y) + ".svg")
...
def visual_measures(args, prediction, figure=True):
    ...
    if figure:
        ...
        fig3.savefig("/home/kudzia/results/SHNet/result_RTOP_cross_red_" + args.net + "_" + str(args.b_x) + "_" + str(args.b_y) + ".svg")
        ...
        fig4.savefig("/home/kudzia/results/SHNet/result_ASD_cross_" + args.net + "_" + str(args.b_x) + "_" + str(args.b_y) + ".svg")
        ...
        fig5.savefig("/home/kudzia/results/SHNet/result_CVD_cross_" + args.net + "_" + str(args.b_x) + "_" + str(args.b_y) + ".svg")
        ...
        fig6.savefig("/home/kudzia/results/SHNet/result_GFA_cross_" + args.net + "_" + str(args.b_x) + "_" + str(args.b_y) + ".svg")
    ...
    if args.b_y == 1000:
        ...
        if figure:
          ...
          fig1.savefig("/home/kudzia/results/SHNet/result_FA_" + args.net + "_" + str(args.b_x) + "_" + str(args.b_y) + ".svg")
          ...
          fig2.savefig("/home/kudzia/results/SHNet/result_MD_" + args.net + "_" + str(args.b_x) + "_" + str(args.b_y) + ".svg")
...
def epoch_measures(args, input_size):
    ...
    if args.b_y == 1000:
        ...
        GFA_a = []
        for i in range(args.num_epochs):
            model = model_dir + str(args.net) + '_hansen_' + str(int(args.b_x/1000)) + '_' + str(int(args.b_y/1000)) + '_' + str(
                (i + 1) * 10) + '.pt'
    ...
    plt.savefig("/home/kudzia/results/SHNet/barplot_per_epoch_hansen_"+str(args.net)+'_'+str(args.b_x)+'_'+str(args.b_y)+".svg")


def main():
    # Experiment settings
    parser = argparse.ArgumentParser(description='Harmoniation experiments')
    parser.add_argument('-pe', '--per_epoch', type =bool, default=True, help='show barplot per epoch')
    parser.add_argument('-ne', '--num_epochs', type =int, default=5, help='')
    parser.add_argument('-o', '--order', type=int, default=4, help='SH order (default: 4)')
    parser.add_argument('-bx', '--b_x', type=int, default=1.0, help='b_x(default: 1000)')
    parser.add_argument('-by', '--b_y', type=int, default=1.4, help='b_y(default: 2000)')
